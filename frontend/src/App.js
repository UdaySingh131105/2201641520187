...
export default function App() {
  ...
  const handleShorten = async (e) => {
    ...
    if (!isValid) {
        logger.warn('Shorten attempt failed due to validation errors.', entriesWithErrors.filter(e => e.error));
        return;
    }
    ...
    try {
        ...
        for (const entry of urlEntries) {
            ...
            if (!shortcode) {
                ...
                } while (newHistory.some(item => item.short === newShortcode));
                shortcode = newShortcode;
            }

            const now = Date.now() + newItems.length; // Ensure unique ID
            const validityMs = (entry.validity || 30) * 60 * 1000;
            const newItem = {
                id: now,
                long: entry.longUrl,
                short: shortcode,
                displayUrl: `/${shortcode}`,
                createdAt: now,
        ...
        setUrlEntries([{ id: Date.now(), longUrl: '', shortcode: '', validity: 30, error: '' }]);

    } catch (err) {
      logger.error('Failed to shorten URLs due to an unexpected error.', err);
      setUrlEntries(entries => [{ ...entries[0], error: 'An unexpected error occurred.' }, ...entries.slice(1)]);
    } finally {
      setIsLoading(false);
    }
  };
